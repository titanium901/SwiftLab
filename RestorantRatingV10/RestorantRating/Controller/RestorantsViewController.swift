...
// MARK: - Table view data source
    ...
    override func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCell.EditingStyle, forRowAt indexPath: IndexPath) {
        if editingStyle == .delete {
            ...
            let path = getDocumentsDirectory().appendingPathComponent((restorants[indexPath.row].foto))
            // Нужно удалить фото по пути
            do {
...
func dataFilePath() -> URL {
    return getDocumentsDirectory().appendingPathComponent("Restorants.plist")
}
...
extension RestorantsViewController: UISearchBarDelegate {
    func searchBarSearchButtonClicked(_ searchBar: UISearchBar) {
        ...
        restorantItemsReference.queryOrdered(byChild: "name").observe(.value) { (snapShot) in
            ...
            for rest in snapShot.children {
                ...
                let restarant = Restarant(snapshot: rest as! DataSnapshot)
                if restarant.name.lowercased().contains(query.lowercased()) {
                    newRest.append(restarant)
                }
            }
            self.restorants = newRest
